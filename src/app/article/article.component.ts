...
export class ArticleComponent implements OnInit {
    ...
    let request = new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (this.QueriesService.books.data === undefined) {
          ...
          if (
            this.book.availabilities[0].borrower !== null &&
            this.book.availabilities[0].borrower !== undefined
          ) {
          ...
          this.bookTotalRate =
            Math.round((totalReviews / this.book.reviews.totalCount) * 10) / 10;
        }
      }, this.count);
    });
  }
  ...
  commentProcess() {
    ...
    if (comment.length > 2) {
      this.QueriesService.addReview(
        this.apollo,
        this.target,
        this.myVoteToString[this.myRateVote - 1],
        comment
      );
    }
  }
  ...
  editProcess() {
    ...
    if (this.isEdit) {
      let comment = String(
        (<HTMLTextAreaElement>document.getElementById("editComment")).value
      );

      while (i < this.book.reviews.nodes.length) {
        this.bookId = this.book.reviews.nodes[i].uid;
        this.toEdit = this.book.reviews.nodes[i].comment;
        this.QueriesService.editReview(
          this.apollo,
          this.bookId,
          this.myVoteToString[this.myRateVote - 1],
          comment
    ...
    while (i < this.book.reviews.nodes.length) {
      ...
      this.QueriesService.editReview(
        ...
        this.bookId,
        this.myVoteToString[this.myRateVote - 1],
        this.book.reviews.nodes[i].comment
      );
